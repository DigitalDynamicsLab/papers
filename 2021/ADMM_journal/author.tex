...
\begin{itemize}
    ...
    \item $H$ is a positive definite matrix containing $M$, the block-diagonal matrix of the masses and inertia tensors of the bodies; if stiff
		...
		for instance $H=M-{h^2}\nabla_q\vect{f}-{h}\nabla\vect{f}$ in case of the time stepper in \cite{TasoraAnitescuCMAME10}
...
\[
   \vect{r}_{\text{dual}}^{k+1} = D_\epsilon^T \vect{v}^{k+1} +C \vect{\gamma}_\epsilon^{k+1} + \vect{r} + \vect{y}^{k+1}
\]
...
\begin{align}
	\eta_\rho = \frac{ \norm{\vect{r}_{\text{primal}}^{k}} }{ \norm{\vect{r}_{\text{dual}}^{k}} }  
\end{align}
We define the \textit{balanced normalized} scaling policy as
\begin{align}
	\eta_\rho = \left( \frac{\norm{\vect{r}_{\text{primal}}^{k}}}{ \norm{\vect{z}^k}+\norm{\vect{\gamma}_\epsilon^k} +10^{-11} } \right)
															\left( \frac{\norm{\vect{r}_{\text{dual}}^{k}}}  { \norm{\vect{y}^k } +10^{-11} }  \right)^{-1}
\end{align}
...
\KwOut{ Solution $\vect{\gamma}_\epsilon$, $\vect{v}$ }
...
\While{$i \leq n_{max iters}$}{
    $ \begin{bmatrix}
	  ...
	  $\vect{z}^{k+1} = \Pi_\Upsilon \left(  \vect{\gamma}_\epsilon^{k+1} +(\Theta^k)^{-1} \vect{y}^k \right )$ \;
		...
		$\rho^{k+1} = $ \StepAdjustPolicy{$\rho^{k}$},  $\rho^{k+1} \rightarrow \Theta^{k+1}$ \;
}
%\Return{location}\;
\caption{Simple form of the ADMM algorithm, with no $N$ Schur complement.}
\label{algo:admm1}
\end{algorithm}
...
ALSO MAYBE THIS ALPHA VARIANT WILL BE REMOVED!!!
...
TO DO: CUT THIS "`OPTIMIZATIONS"' SECT, HERE ONLY FOR CUT PASTING
