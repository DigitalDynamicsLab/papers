...
\begin{algorithm}
\DontPrintSemicolon
\KwIn{ Initial approximations $\vect{\gamma}_\epsilon^0$, $\vect{y}^0$ }
\KwOut{ Solution $\vect{\gamma}_\epsilon^\star$, $\vect{v}^\star$, optionally also $\vect{y}^\star$, $\vect{z}^\star$ }
\SetKwFunction{StepAdjustPolicy}{StepAdjustPolicy}
$\alpha^{0} = 1, \quad \Theta=\Theta(\rho), \quad r_{\text{comb}}^{0} = \infty$ \;
\While{$i \leq n_{max iters}$}{
    $\vect{z}^{k+1} = \Pi_\Upsilon \left(  \vect{\gamma}_\epsilon^{k} +(\Theta^k)^{-1} \vect{y}^k \right )$ \;
		$ \begin{bmatrix}
			 H   & D_\epsilon \\
			...
			\end{Bmatrix} $\;
		$\vect{y}^{k+1} = \vect{y}^k + \Theta \left( \vect{\gamma}_\epsilon^{k+1} - \vect{z}^{k+1} \right) $ \;
		$\vect{r}_{\text{primal}}^{k+1} = \vect{\gamma}_\epsilon^{k+1}-\vect{z}^{k+1}$ \;
		$\vect{r}_{\text{dual}}^{k+1}   = \Theta (\vect{z}^{k+1} - \vect{z}^{k})$ \;

		\If{$\norm{\vect{r}_{\text{primal}}^{k+1}} < \epsilon_{\text{primal}}$ and $\norm{\vect{r}_{\text{dual}}^{k+1}} < \epsilon_{\text{dual}}$ }{
			break \;
		}
		
		$r_{\text{comb}}^{k+1}= \Theta^{-1} \norm{\vect{y}^{k+1}-\vect{y}^{k}}_2 + \Theta \norm{\vect{\gamma}_\epsilon^{k+1}-\vect{\gamma}_\epsilon^{k} }_2$
		
		\If {$r_{\text{comb}}^{k+1} < \epsilon_r r_{\text{comb}}^{k}$ } {
            $\alpha^{k+1} = \frac{ 1 + \sqrt{1 + 4 (\alpha^{k})^2} }{ 2 } $ \;
            $\vect{\gamma}_\epsilon^{k+1} = \vect{\gamma}_\epsilon^{k} + \frac{\alpha^{k}-1}{\alpha^{k+1}} (\vect{\gamma}_\epsilon^{k+1} - \vect{\gamma}_\epsilon^{k}) $ \;
            $\vect{v}^{k+1} = \vect{v}^{k} + \frac{\alpha^{k}-1}{\alpha^{k+1}} (\vect{v}^{k+1} - \vect{v}^{k}) $  \;
						$\vect{y}^{k+1} = \vect{y}^{k} + \frac{\alpha^{k}-1}{\alpha^{k+1}} (\vect{y}^{k+1} - \vect{y}^{k}) $  \;
		}
    \Else{
            $\alpha^{k+1} = 1 $ \;
            $r_{\text{comb}}^{k+1} = \frac{1}{\epsilon_r} r_{\text{comb}}^{k} $ \;
    }
}
